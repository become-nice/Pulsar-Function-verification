...
def consumer_data(topic, process_index, partitions, subscription_name):
    client = pulsar.Client('pulsar://localhost:6650')
    consumer = client.subscribe(topic, subscription_name, consumer_type=ConsumerType.KeyShared)
    flag = True
...
if __name__ == "__main__":
    p1 = Process(target=consumer_data, args=("test", "p1", ["test-partition-0", "test-partition-1"], "sub1",))
    p2 = Process(target=consumer_data, args=("test", "p2", "", "sub2",))
    p1.start()
    time.sleep(10)
    p2.start()